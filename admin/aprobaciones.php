...
require_any_role(['superadmin','admin']); csrf_check();
...
$pendingProviders = $pdo->query("SELECT p.id, u.email, p.display_name, p.created_at FROM providers p JOIN users u ON u.id=p.user_id WHERE p.status='pending' ORDER BY p.id DESC")->fetchAll();
$pendingWholesale = $pdo->query("SELECT s.id, u.email, s.display_name, s.wholesale_status FROM sellers s JOIN users u ON u.id=s.user_id WHERE s.wholesale_status='pending' ORDER BY s.id DESC")->fetchAll();
...
if (!$pendingProviders) echo "<p>No hay pendientes.</p>";
  ...
  echo "<table border='1' cellpadding='6' cellspacing='0'><tr><th>ID</th><th>Email</th><th>Nombre</th><th>Acción</th></tr>";
  foreach($pendingProviders as $p){
    echo "<tr><td>".h((string)$p['id'])."</td><td>".h($p['email'])."</td><td>".h($p['display_name'])."</td><td>
...
if (!$pendingWholesale) echo "<p>No hay pendientes.</p>";
  ...
  foreach($pendingWholesale as $s){
    echo "<tr><td>".h((string)$s['id'])."</td><td>".h($s['email'])."</td><td>".h($s['display_name'])."</td><td>
      <form method='post' style='margin:0'>
        <input type='hidden' name='csrf' value='".h(csrf_token())."'>
